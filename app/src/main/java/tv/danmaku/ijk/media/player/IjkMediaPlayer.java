...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    // @{
    // MOD(tony)
    // RTP HDR EXT CALLBACK
    protected static final int MEDIA_RTP_HDR_EXT = 10002;
    // JSON-RPC CALLBACK
    protected static final int MEDIA_JSON_RPC_ANS = 10003;
    // @}
    ...
    public void setLogEnabled(boolean enable) {
        // do nothing
        native_setLogLevel(enable?IjkMediaPlayer.IJK_LOG_DEBUG:IjkMediaPlayer.IJK_LOG_SILENT);
    }
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            // {
            // MOD(tony)
            case MEDIA_RTP_HDR_EXT:
                if (msg.obj == null) {
                    player.onRtpHdrExtAvailable(null, msg.arg1);
                } else {
                    player.onRtpHdrExtAvailable((ByteBuffer) msg.obj, msg.arg1);
                 }
                return;
            case MEDIA_JSON_RPC_ANS:
                if (msg.obj == null) {
                    player.onRpcResult(null, msg.arg1);
                } else {
                    player.onRpcResult((String) msg.obj, msg.arg1);
                }
                return;
            // }
    ...
    // MOD(tony):
    // JSON-RPC
    public synchronized void asyncRpc(String message) {
        if (message == null || message.length() == 0) {
            throw new IllegalArgumentException("asyncRpc: The length of rpc message must be > 0");
        }
        if (!message.endsWith("\r\n")) {
            message += "\r\n";
        }
        native_async_rpc(message);
    }

    private native void native_async_rpc(final String message);
    // @}
